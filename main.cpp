...
public:
    ...
    int addClient(const std::string& firstName, const std::string& lastName, const std::string& email) //создаём клиента и возвращаем его clientId
    {
        ...
        create3 = "INSERT INTO person(first_name, last_name, e_mail) VALUES('" + firstName + "' , '" + lastName + "' , '" + email + "');";
        create1 = "select id from person where ((first_name='"
            "" + firstName + "') and (last_name ='"
            "" + lastName + "') and (e_mail ='"
            "" + email + "')); ";
       
        tx3.exec(create3);
        id1 = tx3.query_value<int>(create1);
        tx3.commit();
        return id1;
    }
    ...
    void addPhoneNumber(int clientId, const std::string& phoneNumber)
    {
        ...
        std::string create2;
        pqxx::work tx1{ connection1 };
        create2 = "INSERT INTO telefon (id_person, telefon) values ('" + std::to_string(clientId) + "','" + phoneNumber + "');";
        tx1.exec(create2);
        tx1.commit();
    ...
    {
        std::string create1 = "update person set first_name = '" + firstName + "', last_name ='" + lastName + "', e_mail = '" + email + "' "
            "where ((id='"
    ...
    void removeClient(int clientId)
    {
        ...
        pqxx::work tx1{ connection1 };
        create2 = "delete from telefon where (id_person='" + std::to_string(clientId) + "');";
        create3 = "delete from person where (id='" + std::to_string(clientId) + "');";
        tx1.exec(create2);
        tx1.exec(create3);
        tx1.commit();
